...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    lld-18 \
    llvm-18 \
    # Cross-compiler (needed by some Kbuild scripts even with LLVM=1)
...
RUN ln -sf /usr/bin/clang-18 /usr/bin/clang && \
    ...
    ln -sf /usr/bin/llvm-size-18 /usr/bin/llvm-size && \
    ln -sf /usr/bin/llvm-strings-18 /usr/bin/llvm-strings && \
    ln -sf /usr/bin/llvm-readobj-18 /usr/bin/llvm-readobj && \
